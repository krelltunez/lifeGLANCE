...
- **Versioning** — the native version is *not* read from `package.json`; keep
  the display version in sync manually when releasing:
  - **Display version** (currently `2.3.3`): Android `versionName`,
    iOS `MARKETING_VERSION`.
  - **Build number** (currently `1`): Android `versionCode`,
    iOS `CURRENT_PROJECT_VERSION`. These must increment on every store upload
    and are managed independently of the semver display version.
